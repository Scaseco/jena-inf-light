...
fuseki-plugin: ## Create the self-contained SameAs Fuseki Plugin JAR
	$(MCCS) $(POM) package -Pbundle -pl :jena-inf-pkg-fuseki-plugin -am $(ARGS)
	file=`find '$(CWD)/jena-inf-pkg-fuseki-plugin/target' -name '*-fuseki-plugin*.jar'`
	printf '\nCreated package:\n\n%s\n\n' "$$file"
...
release-github: ## Create files for Github upload
	...
	file=`find '$(CWD)/jena-inf-pkg-fuseki-plugin/target' -name '*-fuseki-plugin*.jar'`
	$(call loud,cp "$$file" "jena-inf-fuseki-plugin-$$ver.jar")
	$(call loud,gh release create v$$ver "jena-inf-fuseki-plugin-$$ver.jar")
